...
if __name__ == "__main__":
    ...
    time.sleep(5)
    # Using the while loop to send a request to do the sweep and send the missiles to missiles1.json
    ...
    ╚══════╝ ╚══╝╚══╝ ╚══════╝╚══════╝╚═╝          ╚═╝    ╚═╝  ╚═╝╚═╝  ╚═══╝╚═════╝     ╚══════╝
        ...
        The purpose of the next two tables is do two different sweeps so we can calcualte
        all of the missile information such as bearing, speed, altitude drop rate etc.
    ...
    """
        - Find the middle points (NOT MIDDLE ANYMORE .75 of the line) of the line using st_line_interpolate_point from the missiles_intersects.missile_line
        - id SERIAL PRIMARY KEY
        - missile_id INT
        - danger_closing boolean
        - missile_line geometry
    ...
    with DatabaseCursor(".config.json") as cur:
        ...
        cur.execute(
            "INSERT INTO numb_sec_reach_missile (missile_id, missiles_Intersects_middle, distance, time) SELECT point_to_shoot.missile_id, point_to_shoot.missiles_Intersects_middle, point_to_shoot.distance, point_to_shoot.distance / 49950 FROM point_to_shoot"
        )
